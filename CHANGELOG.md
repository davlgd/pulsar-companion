...
- `pulsar-client` 1.17 and 1.18 can discard deliveries during native reader creation, before the JavaScript listener receives them. For `--since earliest` and past timestamps, the CLI starts at the end of the topic and requests a timestamp replay after creation, using timestamp 0 for `earliest`. This mitigates the observed startup loss within the history still retained; it does not guarantee loss-free delivery. The underlying C++ client can report a successful seek even when repositioning failed, so completion alone does not confirm a replay. `--since latest` and future timestamps do not replay, and messages published during creation may not appear. Updating from 1.17 to 1.18 does not fix this creation-window defect. Tracked in [#3](https://github.com/davlgd/pulsar-companion/issues/3), which records the measurements and what a real fix would need.
- Both replayed positions — `--since earliest` and a past `--since <timestamp>` — start at the end of the topic and reposition, so a live message delivered during that startup can be printed before the older history that follows: the first lines are not guaranteed to be chronological. Redeliveries the repositioning causes are filtered on a best-effort basis, bounded to 10000 message ids, and the CLI prints every callback it is handed rather than discarding any of them.
