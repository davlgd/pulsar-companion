...
describe('empty option values', () => {
  ...
  // The last positional is a value, so an empty one there must be refused
  // just like the matching flag: the two are documented as equivalent.
  for (const [label, argv, slot] of [
    ['a lone empty topic', ['--send', 'x', ''], 'topic'],
    ['a trailing empty subscription', ['myTopic', ''], 'sub'],
    ['whitespace only', ['--send', 'x', '   '], 'topic']
  ]) {
    test(`${label} is rejected, like the flag would be`, async () => {
      await assert.rejects(() => new ArgumentParser(argv).validateArgs(),
        new RegExp(`Positional ${slot} needs a value`));
    });
  }

  // A flag takes precedence, so a positional it replaces is never read and
  // its emptiness is moot — refusing it would contradict that precedence.
  for (const [label, argv, slot, value] of [
    ['--topic', ['--send', 'x', '--topic', 'actual', ''], 'topic', 'actual'],
    ['--sub', ['--sub', 'actual', 'myTopic', ''], 'sub', 'actual']
  ]) {
    test(`an empty trailing positional replaced by ${label} is accepted`, async () => {
      const parser = new ArgumentParser(argv);
      await parser.validateArgs();
      assert.equal(parser.getSetting(slot), value);
    });
  }

  // The flag's own rule still governs it, even with a usable positional.
  test('an empty flag stays invalid even when a positional could supply it', async () => {
    await assert.rejects(() => new ArgumentParser(['--send', 'x', '--topic', '', 'valid']).validateArgs(),
      /Option --topic needs a value/);
  });

  // The key slot carries data, not a name, so an empty one is a real key.
  test('an empty trailing key is accepted, matching --key ""', async () => {
    const parser = new ArgumentParser(['--send', 'x', 'myTopic', 'mySub', '']);
    await parser.validateArgs();
    assert.equal(parser.getValue('key'), '');
  });
});
