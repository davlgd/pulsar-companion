...
describe('producer lifecycle', () => {
  ...
  // create() without a compression argument used to reference CONFIG without
  // importing it, and then to pass a name pulsar-client does not accept.
  test('create() without a compression argument sends a canonical name', async () => {
    const { client, recorded } = clientDouble();
    const producer = new PulsarProducer(client, CONFIG);
    await producer.create('persistent://tenant/ns/t');
    assert.equal(recorded.config.compressionType, CONFIG.compressionTypes.NONE);
    assert.ok(Object.values(CONFIG.compressionTypes).includes(recorded.config.compressionType),
      'the default must be a name the client accepts');
  });
